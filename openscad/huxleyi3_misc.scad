	...
	color("pink")difference(){
		...
		translate([13,0,0]){
			...
			translate([-perfil-0.5,perfil+5,2.5])
				cube([perfil+0.2,45,perfil+5]);
			translate([-perfil,2.5,2.5])
                cube([3*perfil,perfil+0.2,perfil+5]);
			// esquinas biseladas
			...
			translate([(perfil/2)+2.5,-5,(perfil/2)+2.5])
                rotate([-90,0,0])cylinder(h=10, r=1.6, $fn=50);
...
module spool_upper_crow(perfil = 22){
	...
	color("pink")difference(){
		...
		translate([2.5,2.5,2.5])
            cube([perfil+0.2,1.2*perfil,perfil+5]);
		// esquinas biseladas
		...
		translate([0,-perfil-6.5,-5])
            cube([2*perfil,perfil,1.5*perfil]);
	}
}
...
// l = length
module slot_clamp(l=10, hole=true){

	difference(){
		union(){
			cube([l,8,4]);
			translate([0,1.5,4])cube([l,5,1.7]);
		}
		if (hole){	
			translate([l/2,4,-2])union(){
