...
module portamotor_14(ht=5){
	difference(){
        ...
        // eje varilla roscada M5
        %translate([13,13,-2])
            cylinder(h=ht+5, r=2.6, $fn=50);
        /*translate([-2.5,13,-2])
            cylinder(h=ht+5, r=3.2, $fn=50);
        %translate([13,-2.5,-2])
            cylinder(h=ht+5, r=3.2, $fn=50);*/
	}
	// porta barra lisa
...
module hotendhead(filament=true,bcoupling=true){
    ...
    union(){
        ...
        if (bcoupling)
            translate([0,0,0])cylinder(h=17.75,r=4,$fn=60);
...
module hollow(rd=5,lg=35,wd=20,hg=20){
	...
	hull(){
		translate([-lg/2,wd/2,-5])cylinder(h=hg,r=rd,$fn=10*rd);
		translate([lg/2,wd/2,-5])cylinder(h=hg,r=rd,$fn=10*rd);
		translate([lg/2,-wd/2,-5])cylinder(h=hg,r=rd,$fn=10*rd);
		translate([-lg/2,-wd/2,-5])cylinder(h=hg,r=rd,$fn=10*rd);
	}
